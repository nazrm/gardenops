...
Do not configure the OpenClaw stdio bridge. It is disabled because a shared
bearer and OpenClaw agent filter cannot prove which Matrix room and sender
caused a tool invocation. Both bridge startup and its REST authorization path
fail closed until OpenClaw can supply immutable, connector-authenticated source
provenance for GardenOps to verify on every request. Continue using the
dedicated Matrix worker, which validates the configured room and sender before
dispatching assistant actions.
