...
| 2 | Pending publication | Pending exact-head review | Rerun required after audit-identity and evidence-contract remediation | Rerun required: the matrix now adds six fresh-context read-only permutation probes before the six-profile ordered Phase 2 shared-state choreography | Pending exact GPT-5.6 Sol Ultra gate | Pending | Read-only permutation probes do not establish mutation-order independence; external weather-adapter variants remain Phase 7; complete accessibility remains Phase 8; measured performance and final closure remain Phase 9 |
