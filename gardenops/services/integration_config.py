...
def agent_api_auth_context(db: DbConn, request: Request) -> AuthContext | None:
    """Reject the legacy REST-over-MCP principal until source attestation exists."""
    ...
    del db
    # A bearer proves which local process knows the shared secret, but not which
    # connector event caused an agent to make this request.  Do not turn it into
    # a user authorization context without immutable, connector-authenticated
    # room and sender provenance.
    raise HTTPException(
        status_code=403,
        detail="GardenOps agent API requires authenticated source provenance",
    )
