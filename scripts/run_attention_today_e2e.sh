...
if [[ -z "${GARDENOPS_ATTENTION_E2E_TEST_URL:-}" ]]; then
  echo "GARDENOPS_ATTENTION_E2E_TEST_URL is required" >&2
  exit 2
fi
...
export GARDENOPS_LOGS_DIR="${GARDENOPS_LOGS_DIR:-/tmp/gardenops-attention-e2e-logs}"
export DATABASE_URL="$GARDENOPS_ATTENTION_E2E_TEST_URL"
export UV_CACHE_DIR="${UV_CACHE_DIR:-/tmp/gardenops-uv-cache}"

BACKEND_PORT="${GARDENOPS_ATTENTION_E2E_BACKEND_PORT:-8000}"
FRONTEND_PORT="${GARDENOPS_ATTENTION_E2E_FRONTEND_PORT:-5173}"
export GARDENOPS_VITE_PROXY_TARGET="http://127.0.0.1:${BACKEND_PORT}"
RUN_DB_AS_POSTGRES="${GARDENOPS_ATTENTION_E2E_RUN_DB_AS_POSTGRES:-0}"
...
run_db_command() {
  if [[ "$RUN_DB_AS_POSTGRES" == "1" ]]; then
    sudo -u postgres env \
      ...
      GARDENOPS_ATTENTION_E2E_ALLOW_TRUNCATE="$GARDENOPS_ATTENTION_E2E_ALLOW_TRUNCATE" \
      ...
      UV_CACHE_DIR="${GARDENOPS_ATTENTION_E2E_POSTGRES_UV_CACHE_DIR:-/tmp/gardenops-uv-cache-postgres}" \
      "$@"
    return
  fi
  "$@"
...
run_db_command uv run python scripts/seed_attention_today_e2e.py
...
BASE_URL="http://127.0.0.1:${FRONTEND_PORT}" node scripts/check_attention_today_e2e.cjs
