...
assertIncludes(apiSource, "export async function movePlotsToAreaApi", "missing existing-plot move API");
assertIncludes(apiSource, "containers/from-plots", "plot move API must target the selected area");
...
assertIncludes(panelSource, "onMovePlotsToArea", "missing selected-plot move callback");
assertIncludes(panelSource, "buildMoveSelectedPlotsForm", "missing move-selected-plots action");
...
assertIncludes(appSource, "movePlotsToAreaApi", "app must convert existing plots through the canonical API");
assertIncludes(appSource, "state.selectedPlotIds.clear()", "successful plot moves must clear stale map selection");
